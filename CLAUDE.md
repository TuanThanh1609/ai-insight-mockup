...
**File đã sửa (Tasks 1–5, 8):**
- `src/components/overview/PillarConversation.jsx` — Thêm: Junk Lead bar, Conversion Rate, Top Products chips, Top Campaigns list, import `mockCampaigns`
- `src/components/overview/PillarStaffEval.jsx` — Thêm: Attitude breakdown (Tốt/TB/Kém), Top Mistakes, Top Selling Scenarios
- `src/components/overview/PillarPostPurchase.jsx` — Thêm: Satisfaction breakdown (Hài lòng/TB/Không HL), Refer rate bar, Urgency ⚠️ alert chip, fix commentary label
- `src/components/overview/PillarFeedback.jsx` — Thêm: Đối thủ cạnh tranh (với ⚠️ khi >10%), Tiêu chí so sánh pills, Thắc mắc phổ biến (pain points + objections), Xu hướng quan tâm (Budget + Location)
- `src/components/overview/ChartsSection.jsx` — Mở rộng grid: 2→4 cột; thêm Top Sản Phẩm + Top Khu Vực mini bar charts

**File đã sửa (Tasks 6–7):**
- `scripts/seed-conversations.js` — Thêm 15 field pools mới (satisfaction, competitor, pain_point, objection, attitude, mistake, scenario, budget, location, segment, frustration, returning, ...); fix `rowSeed is not defined` → dùng `rng()`
- `scripts/truncate-conversations.js` — Fix `DROP TABLE` → `TRUNCATE TABLE ... RESTART IDENTITY CASCADE` để tránh dependency error

**Bug đã fix:**
- `rowSeed is not defined` trong seed-conversations.js — `sr(rowSeed + N * 7919)` dùng biến chưa khai báo → thay bằng `Math.floor(rng() * POOL.length)`
- `DROP TABLE` dependency error — đổi sang `TRUNCATE TABLE ... CASCADE`
- Duplicate `CREATE TABLE` block thừa trong seed script
- PillarPostPurchase commentary label: `"% hài lòng"` → `"% quay lại"` (metric đúng)

**Seed result:**
- Supabase: **3,075 rows** (42 templates × 50–99 rows/template)
- Export JSON: **3,128 rows**, 2.82 MB, 13 new fields × row

**Metric coverage mới:**

| Pillar | Metrics mới |
|--------|------------|
| A — Hội thoại | Junk Lead %, Tỉ lệ chốt đơn, Top Products, Top Campaigns |
| B — NV Tư vấn | Attitude breakdown, Top Mistakes, Selling Scenarios |
| C — Sau mua | Satisfaction breakdown, Refer rate, Urgency ⚠️ |
| D — Feedback | Đối thủ, Tiêu chí, Pain Points, Xu hướng (Budget/Location) |
| Charts | Top Products bar + Top Locations bar |
...
- **File đã tạo:**
  - `src/pages/OverviewDashboard.jsx` — Page layout CEO Command Center (3-row grid)
  - `src/components/overview/KpiStrip.jsx` — 4 KPI cards: Tổng hội thoại / Chi tiêu Ads / Doanh thu / ROAS
  - `src/components/overview/ChartsSection.jsx` — Revenue vs Spend bar chart + Temperature funnel
  - `src/components/overview/AlertsPanel.jsx` — CriticalAlertsPanel + RuleCommentary wrapper
  - `src/components/overview/RuleCommentary.jsx` — Hybrid rule-based commentary engine (rule instant + AI on-demand) cho 4 pillars
  - `src/components/overview/PillarCard.jsx` — DRY wrapper component cho 4 pillars
  - `src/components/overview/PillarConversation.jsx` — Pillar A: Lead funnel (Nóng/Ấm/Lạnh) + Phone rate
  - `src/components/overview/PillarStaffEval.jsx` — Pillar B: NV tư vấn — chốt đơn / bỏ sót / im lặng
  - `src/components/overview/PillarPostPurchase.jsx` — Pillar C: Donut khách quay lại / không quay lại
  - `src/components/overview/PillarFeedback.jsx` — Pillar D: Sentiment bar + top concerns chips
- **File đã sửa:**
  - `src/App.jsx` — Thêm route `/insight/overview` → `<OverviewDashboard />`
  - `src/components/layout/Sidebar.jsx` — Thêm nav item "Tổng Quan" (LayoutDashboard icon, badge "NEW")
  - `src/components/overview/AlertsPanel.jsx` — Fix named import: `CriticalAlertsPanel` (named, không default)
- **Thay đổi UX:**
  - CEO Command Center: 3-row layout — KPI Strip / Charts+Alerts / 4 Pillar Cards
  - Hybrid Commentary: rule-based (instant) + AI button "Phân tích chi tiết với AI ↗" cho từng pillar
  - 4 Pillars: A (Hội Thoại) / B (NV Tư vấn) / C (Sau mua) / D (Feedback KH)
  - 7 ngày auto-filter từ `Date.now()`, fallback toàn bộ data
  - ROAS color-coded: ≥3× xanh / ≥1.5× vàng / <1.5× đỏ
- **Bug đã fix:** `CriticalAlertsPanel` là named export (không phải default) → sửa import trong AlertsPanel.jsx
...
- **File đã sửa:**
  - `src/lib/medicalService.js` — Rewrite `loadConversations()`: `supabase-conversations.json` là flat array 6202 records (đánh số 0-6201), không phải nested object. `loadConversations()` trước đây dùng `supabaseConversationsRaw[tid]` → undefined → conversations = [] → NaN hàng loạt. Fix: traverse flat array, index by `template_id` field.
  - `src/components/medical/LeadsQualityDashboard.jsx` — 4 fallback values từ string (`'68'`, `'23'`, `'12'`, `'18'`) → number (`0`). Key `'phoneCollected'` → `'phoneCollectionRate'` (đúng với `getLeadsKPIWithTrend()` output).
- **Root Causes tìm được:**
  1. **CRITICAL:** `supabase-conversations.json` flat array (6202 rows đánh số 0-6201) ≠ nested object → `loadConversations()` luôn trả về `[]`
  2. **CRITICAL:** `LeadsQualityDashboard` dùng fallback string `'68'`/'23'`/'12'`/'18'` → `style={{ width: '68%' }}` → NaN khi nhân
  3. **BUG:** Key `'phoneCollected'` không khớp output `'phoneCollectionRate'` từ `getLeadsKPIWithTrend()`
- **Metric thực từ data (Thời trang):**
  - 823 conversations được load (6 templates × fsh-1 → fsh-6)
  - Junk Lead: 3% | Phone Collection: 0% (Supabase chỉ có `phone_status = "Chưa cho"`) | Conversion: 7%
  - Health Score: 5.9/10
- **Verify:** Screenshot dashboard + grep snapshot → 0 NaN occurrences ✅
- **Còn cần làm:** Không
...
- **Bug nghiêm trọng phát hiện + fix:**
  - `rounded-[--radius-lg]` trong Tailwind v4 **KHÔNG resolve được CSS variable** ở runtime → ra `0px` (vuông hoàn toàn)
  - Root cause: Tailwind arbitrary value syntax `rounded-[--var]` không hỗ trợ CSS variable reference
  - Fix: Thay **68 files** `rounded-[--radius-*]` → class chuẩn Tailwind: `rounded-lg`/`rounded-xl`/`rounded-md`/`rounded-sm`
  - Perl sed: `perl -i -pe 's/rounded-\[--radius-lg\]/rounded-lg/g; ...'`
- **Tổng thay đổi:** 68 files × tất cả border-radius violations
- **Verify computed CSS (Playwright):**
  - `rounded-lg` → **14px** ✅ (Disease filter cards, KPI cards)
  - `rounded-md` → **10px** ✅ (Buttons, icon containers)
  - `rounded-sm` → **6px** ✅ (Checkbox tick-boxes)
- **Trang đã verify:** Landing (`/`), Dashboard Ads (`/insight/dashboard`), Ads Optimization (`/insight/ads-optimization`), Khám Bệnh (`/insight/medical-checkup`) — tất cả bo góc đúng
- **Còn cần làm:** Không
...
- **File đã tạo:**
  - `src/pages/ImprovementRoadmapPage.jsx` — Route page với header (Quay lại / Xuất PDF), đọc state từ localStorage
  - `src/components/medical/ImprovementRoadmapContent.jsx` — Split-panel layout (40/60), total progress bar, filter chips, 4 metric cards
  - `src/components/medical/JobListPanel.jsx` — Left panel: filter chips + 4 phase accordions (color-coded) + job cards với progress ring + priority border-left
  - `src/components/medical/JobCard.jsx` — Single job card: completion ring + title + priority/severity badges + done/total count
  - `src/components/medical/JobDetailPanel.jsx` — Right panel: job header với large progress SVG + projected metrics (before→after ↑) + sub-task list với checkbox + complete-all button
- **File đã sửa:**
  - `src/App.jsx` — Thêm route `/insight/improvement-roadmap` → `<ImprovementRoadmapPage />`
  - `src/components/medical/MedicalResultStep.jsx` — Thay "Xuất báo cáo PDF" bằng nút "Lộ Trình Cải Thiện" (Map icon, variant primary) → save state → navigate
  - `src/lib/medicalService.js` — Thêm `generateImprovementRoadmap(diseases, conversations)` trả về 4 phases × 3 jobs × 2-4 sub-tasks + projected metrics theo disease data thực; thêm `getRoadmapChecked()`/`saveRoadmapChecked()` cho localStorage persist
- **Thay đổi UX:**
  - Nút "Lộ Trình Cải Thiện" nằm bên phải nút "Xem Tóm Tắt" trên meta bar (cùng variant primary, không còn tertiary)
  - Split panel: click job card → right panel hiện sub-tasks với checkbox tick được
  - Progress bar tổng + per-phase mini ring + per-job completion %
  - Checklist persist qua localStorage (key: `roadmap-checked-{industry}`)
  - Chỉ số dự tính cải thiện mỗi Phase header: Lead Quality, Junk Lead %, Response Speed, Phone Collection %, Objection Handling, Conversion %, Overall Health, ROAS
- **Thiết kế:** Khớp Stitch references `34d3e8cf...` (overview) + `a33e29fb...` (split detail)
- **GitHub:** chưa push
...
- **URL:** https://audit.cdp.vn/insight/medical-checkup
- **File đã tạo:**
  - `src/components/medical/HealthSummaryPanel.jsx` — Slide-in panel "Bảng Tóm Tắt Sức Khỏe Hội Thoại": Header sticky (Đóng · Tóm Tắt · Xuất PDF) + Overall summary block (SVG score circle + progress + prose diagnosis 2-3 dòng) + 3 KPI chips (Junk/SĐT/Chốt đơn) + Severity breakdown bar + Danh sách 9 nhóm bệnh (icon + score + severity badge + key concern prose + summary chips) + Top 3 HIGH priority actions. Text-first, 0 chart chính. ESC + backdrop click đóng panel. `window.print()` cho PDF export.
- **File đã sửa:**
  - `src/lib/medicalService.js` — Thêm `generateOverallSummary(diseases)` trả về `{ headline, diagnosis, severityCounts, worstLabel, worstScore, bestLabel, bestScore, avgScore }`
  - `src/components/medical/MedicalResultStep.jsx` — Thêm button "Xem Tóm Tắt" (variant="primary", Deep Rust) cạnh "Xuất báo cáo PDF"; thêm state `showSummary`; render `<HealthSummaryPanel>` khi `showSummary === true`
- **Thay đổi UX:**
  - Button "Xem Tóm Tắt" nằm trên meta bar của result step — CEO/CMO click ngay khi thấy dashboard, scan trong 3 giây trước khi drill down
  - Panel slide-in từ phải (480px), dashboard visible bên dưới — user vừa xem summary vừa thấy chart chi tiết
  - "Chi tiết →" trong mỗi disease card → `scrollIntoView({ behavior: 'smooth' })` tới `disease-{id}` trên dashboard rồi đóng panel
- **GitHub:** chưa push
- **Còn cần làm:** Push GitHub; "Xuất báo cáo PDF" vẫn là stub `alert()` — cần implement `window.print()` thực sự (panel đã có CSS `@media print`)
...
- **File đã sửa:**
  - `scripts/seed-conversations.js` — Thêm 4 fields mới vào 7 templates `*-3` (fsh-3, mbb-3, cos-3, spa-3, rls-3, fb-3, trv-3): `scenarios` (4 loại kịch bản), `chot_don` (boolean), `missed_conv` (boolean), `silent_cust` (boolean). Update `generateRows()` cho đúng 7 templates.
  - `src/lib/mockDataService.js` — Thêm `FIELD_DISPLAY_NAMES` + `ENUM_FIELDS` cho 4 fields mới; thêm `injectStaffEvalColumns()` chèn 4 extra columns vào Supabase data cho 7 staff-eval templates.
  - `src/components/insight/DynamicMetricsGrid.jsx` — Thêm `detectCardType('scenario-bars')` + component `ScenarioBarsCard` (Bar chart với chot rate % per scenario); cập nhật `DonutChart` label cho 3 boolean fields mới (Bỏ sót / Im lặng / Chốt đơn).
  - `src/lib/aiService.js` — Update `SYSTEM_PROMPT`: pattern #3 "Đánh Giá Nhân Viên Tư Vấn" bắt buộc 5 cột (Thái độ tư vấn, Kịch bản bán hàng, Chốt đơn thành công, Bỏ sót hội thoại, Khách im lặng). AI sinh insight theo đúng 5 cột khi tạo từ popup.
  - `src/data/supabase-conversations.json` — Regenerate 6202 rows với data mới.
- **4 fields mới (7 ngành × staff-eval template):**
  - `scenario` — 4 giá trị: Tư vấn chi phí / Khai thác thông tin / Ưu đãi cá nhân hóa / Giải quyết vấn đề
  - `chot_don` — Boolean: KH đã được chốt đơn thành công
  - `missed_conv` — Boolean: Sale bỏ sót hội thoại (không rep, rep trễ, quên follow-up)
  - `silent_cust` — Boolean: Khách im lặng (seen không rep, 1 câu không tương tác tiếp)
- **Chart types mới:**
  - `ScenarioBarsCard` — Bar chart hiệu suất chốt đơn theo kịch bản: 4 scenarios × bar với tỉ lệ chốt đơn (%) hiển thị trên bar
  - `DonutChart` (Bỏ sót) — SVG donut với label "Bỏ sót / Bình thường"
  - `DonutChart` (Im lặng) — SVG donut với label "Im lặng / Phản hồi"
  - `DonutChart` (Chốt đơn) — SVG donut với label "Chốt đơn / Chưa chốt"
- **AI Prompt Update:** `aiService.js` system prompt bổ sung chi tiết bảng 5 cột bắt buộc cho pattern #3 — AI sẽ sinh đúng 5 fields khi user tạo insight mới qua popup "Tạo Insight bằng AI"
- **TrendChart Cross-Filter Fix:** `InsightTrendChart.jsx` + `InsightDetail.jsx` — khi bấm filter trên block metrics bên dưới, chart phía trên cập nhật đúng: (1) truyền `filteredRows` prop từ `InsightDetail` xuống chart, (2) chart dùng `filteredRows` thay `rows` để detect columns + compute buckets, (3) bỏ uniform scaling approximation vì data đã đúng từ đầu.
- **Data verified:** fsh-3: 138 rows, 4 scenarios đều có, chot_don=7%, missed_conv=5%, silent_cust=4%
- **Còn cần làm:** Không

### 2026-04-01 — Track A: Seed Conversations — Random 50–99 rows/template
- **Trạng thái:** ✅ Xong + Supabase ✅ + Export JSON ✅
- **Phát hiện bug:** `getRowCount()` dùng `sr(seed)` deterministic → tất cả template cùng ngành ra cùng số. Fix: đổi sang `Math.random()` để mỗi lần seed cho kết quả khác nhau.
- **File đã sửa:**
  - `scripts/seed-conversations.js` — `getRowCount()` dùng `Math.random()` (50–100 rows), script comment + logging update
- **File đã tạo:**
  - `scripts/truncate-conversations.js` — truncate bảng trước khi seed (MCP DDL)
  - `scripts/export-conversations.cjs` — export Supabase → JSON (CommonJS, bypass ESM)
- **Kết quả Supabase:** **3220 rows total** (42 templates, mỗi template random 50–99 rows)
- **Export:** `src/data/supabase-conversations.json` — **1.63 MB**
- **Còn cần làm:** Không
...
- **URL:** https://audit.cdp.vn/insight/settings
- **Root cause:** Supabase JSON có `data_json.row` (row index từ DB) — field này nằm trong `data_json` object, nhưng `buildColumnsFromRows` flatten `data_json` ra → field `row` trở thành data column.
- **Fix:** Thêm `'row'` vào `META_FIELDS` trong `buildColumnsFromRows()` — `row` là row index, không phải business data.
- **Verify:** Template "Phân Tích Nhu Cầu KH" → cột table chỉ còn: Khách hàng | Kênh | Size | Sản phẩm | Nhu cầu cốt lõi | Mức độ quan tâm (không còn "Row") ✅

### 2026-04-01 — Track A: DynamicMetricsGrid v2 — 6 Chart Types + 3 Col Grid + Channel Donut
- **Trạng thái:** ✅ Xong + Build OK (2377 modules) + Deploy production ✅
- **URL:** https://audit.cdp.vn/insight/settings
- **Root cause rollback:** `buildColumnsFromRows()` trong `mockDataService.js` không infer `dataType` → `DynamicMetricsGrid` fallback về mặc định → tất cả cards đều là top-list nhỏ (p-3, font 11px, grid 4 cột).
- **File đã sửa:**
  - `src/lib/mockDataService.js` — Thêm `inferDataType(field, rows)` + gắn `dataType` vào mỗi column; xóa `'platform'` khỏi `META_FIELDS`; thêm `platform: 'Kênh'` vào `FIELD_DISPLAY_NAMES`; thêm `if (f === 'platform') return 'channel'` trong `inferDataType`
  - `src/components/insight/DynamicMetricsGrid.jsx` — Rewrite hoàn toàn: 6 chart types × layout 3 cột; thêm `detectCardType` check cho `dataType === 'channel'`; thêm `ChannelDonut` component (SVG donut Facebook/Zalo với colored pills)
  - `src/components/insight/InsightTrendChart.jsx` — Update type check cho `donut-bool`/`donut-binary`
- **6 Chart Types:** ScoreCard (KPI lớn), DonutChart (SVG boolean), BarChart (top-N ranking), GaugeCard (semi-arc), TemperatureCard (Nóng/Ấm/Lạnh), ChipGrid (demographics)
- **Chart #7 — ChannelDonut:** SVG donut đa segments cho `platform` — Facebook (#3b82f6) / Zalo (#8b5cf6) / Others (#94a3b8), donut center hiện tổng, legend với inline bar, pill row với % badge. Filter click → `field: 'platform'`
- **Layout:** `grid-cols-1 sm:grid-cols-2 lg:grid-cols-3` (3 block/hàng thay vì 4), card padding `p-5`, font header `text-sm`, icon `w-9 h-9`
- **Verify Playwright:** Template "Phân Tích Nhu Cầu KH Thời Trang" → Sản phẩm (Bar Top-N), Size (Bar), Mức độ quan tâm (Temperature Nóng2/Ấm5/Lạnh4), **Kênh (ChannelDonut Facebook55%/Zalo45%)**, Nhu cầu cốt lõi (Bar) ✅
- **Còn cần làm:** Không
...
- **URL:** https://audit.cdp.vn/insight/improvement-roadmap
- **File đã sửa:**
  - `src/lib/medicalService.js` — Thêm `estimatedDays` (1-3 ngày) vào mọi sub-task; thêm `dateStart`/`dateEnd` vào mỗi phase (tính từ hôm nay); mở rộng localStorage model: key `roadmap-status-{industry}` → `{ [subTaskId]: 'todo'|'in-progress'|'done' }` (migration tự động từ old format); thêm helper `_phaseDateRange()`, `_jobEstimatedDays()`, `_phaseEstimatedDays()`; export `getRoadmapStatus()`, `saveRoadmapStatus()`
  - `src/lib/smaxAIService.js` — Thêm `generateRoadmapSubTasks({ disease, job, industry, industryLabel })` + `generateAllRoadmapSubTasks({ jobs, ... })` — gọi SMAX AI với prompt context-aware, cache 24h, parse JSON, fallback null
  - `src/components/medical/JobDetailPanel.jsx` — Rewrite: 3-state lifecycle (todo→in-progress→done→todo), SVG progress ring với %, "Kết quả khi hoàn thành" block, status summary row, estimatedDays hiển thị, AI badge
  - `src/components/medical/JobListPanel.jsx` — Phase header: "Kết quả dự kiến" block (always visible), date range (DD/MM→DD/MM), 4 filter chips (Tất cả/Đang làm/Chưa làm/Hoàn thành)
  - `src/components/medical/JobCard.jsx` — Đổi prop `checkedSubTasks` → `statusMap`
  - `src/components/medical/ImprovementRoadmapContent.jsx` — `Map<id, status>`, 4-state filter logic, `handleToggleStatus()` cycle + persist
- **Thay đổi UX:**
  - Phase card: white card + left-border (4px colored) + hover-shadow — giống reference design
  - Phase header: title (bold 14px) + week badge + date range (Clock icon) + progress bar (140px) + "X/Y việc" count
  - Phase metrics strip (always visible, even when collapsed): nền nhạt 5% màu phase, label "Kết quả dự kiến" + divider màu phase + MetricArrowCard
  - MetricArrowCard: label uppercase (11px semibold), before (15px semibold), SVG arrow, after (18px bold), unit suffix, diff badge (↑↓ với TrendingUp/Down icon + color green/red)
  - Sub-task: 3 trạng thái (□ Chưa làm / ◐ Đang làm / ✓ Hoàn thành) — click để cycle
  - Filter: 4 chip (Tất cả / Đang làm / Chưa làm / Hoàn thành)
  - AI guidance section: Sparkles icon + loader state + AI sub-task cards với border tím
  - Estimated days: "~1 ngày" trong sub-task, "~8 ngày" trong job header
- **Bug fix:** `checkedSubTasks is not defined` — refactoring residue trong JobListPanel JSX (sửa 1 dòng)
- **Thiết kế:** Editorial Precision (Deep Navy / Deep Rust / Vibrant Blue / 8px radius)
- **Còn cần làm:** Không
